...
/**
 * Reusable store import + selection + geofencing experience.
 *
 * Controlled component: the persistent state (analysis `preview`, the set of
 * selected store ids and the per-store `radii`) is owned by the parent and
 * passed in with its setters, so it survives across contexts (the standalone
 * Magasins page and the campaign-creation wizard step). Only ephemeral UI
 * state (chosen file, city filter, in-flight/error) lives here.
 *
 * `onPreviewChange` / `onSelectionChange` / `onRadiiChange` are the raw React
 * setState functions, so passing updater callbacks keeps the handlers stable.
 */
export default function StoreTargetingPanel({
  preview,
  onPreviewChange,
  selectedIds,
  onSelectionChange,
  ...
  const handleAnalyze = async () => {
    ...
    resetSelection();
    try {
      onPreviewChange(await previewStoreImport(file));
    } catch (err) {
      onPreviewChange(null);
  ...
  // Stable identity (only depends on the setter) so the map's marker effect
  // doesn't rebuild markers on every unrelated re-render (e.g. slider moves).
  ...
  const radiusOf = (storeId) => radii[storeId] ?? defaultRadiusKm;

  const handleRadiusChange = (storeId, km) => {
    onRadiiChange((prev) => ({ ...prev, [storeId]: km }));
  };
  ...
  const averageRadius = useMemo(() => {
    ...
    const total = selectedStores.reduce(
      (sum, store) => sum + (radii[store.store_id] ?? defaultRadiusKm),
      0,
    );
    return Math.round((total / selectedStores.length) * 10) / 10;
  }, [selectedStores, radii, defaultRadiusKm]);
    ...
    <>
      ...
      {preview && (
        <>
          ...
          {/* Interactive map — kept in sync with the table's selection, city
              filter and geofencing radii. */}
          <StoreMap
